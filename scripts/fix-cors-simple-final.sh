...
# Script simple para configurar CORS en el API de sandbox
set -e

echo "🔧 Configurando CORS simple en API de sandbox..."
...
configure_cors_for_resource() {
    ...
    for method in $METHODS; do
        if [ "$method" != "OPTIONS" ]; then
            ...
            # Agregar headers CORS a la respuesta del método usando patch-ops
            aws apigateway update-method-response \
                --rest-api-id "$API_ID" \
                --resource-id "$resource_id" \
                --http-method "$method" \
                --status-code 200 \
                --patch-ops op=add,path=/responseParameters/method.response.header.Access-Control-Allow-Origin,value=true op=add,path=/responseParameters/method.response.header.Access-Control-Allow-Headers,value=true op=add,path=/responseParameters/method.response.header.Access-Control-Allow-Methods,value=true
            
            # Agregar headers CORS a la integración usando patch-ops
            aws apigateway update-integration-response \
                --rest-api-id "$API_ID" \
                --resource-id "$resource_id" \
                --http-method "$method" \
                --status-code 200 \
                --patch-ops op=add,path=/responseParameters/method.response.header.Access-Control-Allow-Origin,value="'*'" op=add,path=/responseParameters/method.response.header.Access-Control-Allow-Headers,value="'Content-Type,X-Amz-Date,Authorization,X-Api-Key,X-Amz-Security-Token'" op=add,path=/responseParameters/method.response.header.Access-Control-Allow-Methods,value="'GET,POST,PUT,DELETE,OPTIONS'"
